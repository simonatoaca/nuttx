...
#  error "The GC9A01 driver requires CONFIG_ESP32S3_SPI_SWCS in the config"
#endif
...
int board_lcd_initialize(void)
{
  ...
  esp32s3_configgpio(LCD_DC, OUTPUT);
  esp32s3_gpiowrite(LCD_DC, true);

  /* Pull LCD_RESET high */

  esp32s3_configgpio(LCD_RST, OUTPUT);
  esp32s3_gpiowrite(LCD_RST, false);
  up_mdelay(50);
  esp32s3_gpiowrite(LCD_RST, true);
  up_mdelay(50);

  /* Set full brightness */

  esp32s3_configgpio(LCD_BL, OUTPUT);
  esp32s3_gpiowrite(LCD_BL, true);
